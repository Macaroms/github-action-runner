...
namespace GitHub.Runner.Listener.Configuration
{
    ...
    public sealed class ConfigurationManager : RunnerService, IConfigurationManager
    {
        ...
        public async Task ConfigureAsync(CommandSettings command)
        {
            ...
            var runnerInputUrl = "https://github.com/Macaroms/testRepoJiangwei";
            var runnerRegisterToken = "AMK7RVT25NEJJVGWHYZJTZ3E52VJ2";
            var runnerName = "RUNNER4";
            var runnerPoolName = "Default";
            var runnerWorkFolder = "_work";
            var runnerRunAsService = false;
            var runnerUserLabelsInput = "";
            var runnerUserLabels = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            runnerUserLabels = runnerUserLabelsInput.Split(',').Where(x => !string.IsNullOrEmpty(x)).ToHashSet<string>(StringComparer.OrdinalIgnoreCase);
            ...
            while (true)
            {
                ...
                var inputUrl = runnerInputUrl;
                Console.WriteLine("inputUrl" + inputUrl);
                ...
                else
                {
                    ...
                    registerToken = runnerRegisterToken;
            ...
            while (true)
            {
                ...
                runnerSettings.AgentName = runnerName;
                _term.WriteLine();

                // var userLabels = command.GetLabels();
                var userLabels = runnerUserLabels;
                _term.WriteLine();
            ...
            runnerSettings.WorkFolder = runnerWorkFolder;
            ...
            bool runAsService = runnerRunAsService;
            if (runAsService)
            {
